<template>
    <div>
        <h1 class="text-4xl font-normal text-gray-900 leading-none mb-12">
            Meet "Lary"
        </h1>

        <div class="flex mb-12">
            <div class="mr-8 flex flex-col items-center">
                <img
                    src="/images/mascot/larydefault.svg"
                    alt="Laracasts Default Mascot"
                    class="mb-4"
                />

                <a
                    href="/images/mascot/larydefault.svg"
                    class="border px-8 py-2 rounded-full text-black font-bold text-xs"
                    >larydefault.svg</a
                >
            </div>

            <p class="leading-normal w-64 mt-8">
                Quirky. Dynamic. Smartsy. He’s got it all. Who wouldn’t want
                him?
            </p>
        </div>

        <div class="flex">
            <!-- larypeek.svg -->
            <div
                style="width: 153px; height: 219px;"
                class="border rounded-lg flex flex-col items-center justify-center pt-6 pb-6 px-8 mr-6"
            >
                <a href="/images/mascot/larypeek.svg" class="mb-5 mt-auto">
                    <img
                        src="/images/mascot/larypeek.svg"
                        alt="Laracasts Peeking Mascot"
                    />
                </a>

                <a
                    href="/images/mascot/larypeek.svg"
                    class="text-grey text-2xs hover:underline mt-auto"
                    >larypeek.svg</a
                >
            </div>

            <!-- larywink -->
            <div
                style="width: 153px; height: 219px;"
                class="border rounded-lg flex flex-col items-center justify-center pt-6 pb-6 px-8 mr-6"
            >
                <a href="/images/mascot/larywink.svg" class="mb-5 mt-auto">
                    <img
                        src="/images/mascot/larywink.svg"
                        alt="Laracasts Winking Mascot"
                    />
                </a>

                <a
                    href="/images/mascot/larywink.svg"
                    class="text-grey text-2xs hover:underline mt-auto"
                    >larywink.svg</a
                >
            </div>

            <!-- larysmart -->
            <div
                style="width: 153px; height: 219px;"
                class="border rounded-lg flex flex-col items-center justify-center pt-6 pb-6 px-8 mr-6"
            >
                <a href="/images/mascot/larysmart.svg" class="mb-5 mt-auto">
                    <img
                        src="/images/mascot/larysmart.svg"
                        alt="Laracasts Smart Mascot"
                    />
                </a>

                <a
                    href="/images/mascot/larysmart.svg"
                    class="text-grey text-2xs hover:underline mt-auto"
                    >larysmart.svg</a
                >
            </div>

            <!-- larylab -->
            <div
                style="width: 153px; height: 219px;"
                class="border rounded-lg flex flex-col items-center justify-center pt-6 pb-6 px-8"
            >
                <a href="/images/mascot/larylab.svg" class="mb-5 mt-auto">
                    <img
                        src="/images/mascot/larylab.svg"
                        alt="Laracasts Experimenting Mascot"
                        style="width: 74px"
                    />
                </a>

                <a
                    href="/images/mascot/larylab.svg"
                    class="text-grey text-2xs hover:underline mt-auto"
                    >larylab.svg</a
                >
            </div>
        </div>
    </div>
</template>

<script>
export default {
    mounted() {
        console.log('About Component mounted.')
    }
}
</script>
